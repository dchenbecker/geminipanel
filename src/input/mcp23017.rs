...
impl PanelInputHandler {
    pub fn new(argv: &[String]) -> Result<PanelInputHandler, InputError> {
        let devices = setup_devices(&[DeviceConfig{ dev_path: &argv[1], address: 0x20, polarity_mask: 0x0000, direction_mask: 0xffff },
                                      DeviceConfig{ dev_path: &argv[1], address: 0x21, polarity_mask: 0x0000, direction_mask: 0xffff },
                                      DeviceConfig{ dev_path: &argv[1], address: 0x22, polarity_mask: 0xfc03, direction_mask: 0xffff },
                                      DeviceConfig{ dev_path: &argv[2], address: 0x20, polarity_mask: 0x0000, direction_mask: 0xff00 }])?;
...
mod tests {
    ...
    fn test_compute_set_values() {
        let new_value = compute_new_values(0, &[BitEvent{ bit: 1, value: 1 },
    ...
    fn test_compute_unset_values() {
        let new_value = compute_new_values(0xffff, &[BitEvent{ bit: 1, value: 0 },
    ...
    fn test_compute_mixed_values() {
        let new_value = compute_new_values(0x00ff, &[BitEvent{ bit: 1, value: 0 },
                                                     BitEvent{ bit: 3, value: 1 },
...
impl InputHandler for PanelInputHandler {
    ...
    fn set_output(&mut self, dev_index: usize, bits: &[BitEvent]) -> Result<(), InputError> {
        if dev_index >= self.devices.len() {
            Err(InputError { message: format!("Invalid device index {}. Number of devices {}", dev_index, self.devices.len()) })
        } else {
            let dev: &mut MCP23017 = &mut self.devices[dev_index];
            if bits.iter().any(|event| event.bit > 15) {
                Err(InputError { message: format!("Invalid bit events: {:?}", bits) })
            } else {
...
impl From<LinuxI2CError> for InputError {
    fn from(err: LinuxI2CError) -> InputError {
        InputError { message: format!("I2C Error: {}", err) }
    }
}
...
fn setup_devices(devices: &[DeviceConfig]) -> Result<Vec<MCP23017>, LinuxI2CError> {
    devices.iter().map(|config| {
        setup_mcp23017(&config.dev_path, config.address, config.polarity_mask, config.direction_mask)
    }).collect()
}
